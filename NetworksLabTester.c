...
/*
    ...
    2. Install a  socket instance on Node1 that will connect to Node3.
    3. Install a UDP socket instance on Node2 that will connect to Node4.
    4. Start the  application at time 1s.
    ...
    size of the  instance over time. Use gnuplot/matplotlib to visualise plots of cwnd vs time.
    8. Mark points of fast recovery and slow start in the graphs.
    9. Perform the above experiment for  variants Tahoe, Reno and New Reno,
...
{
    // TODO: change latency with each experiment
  std::string lat = "2ms";
    // TODO: change capacity to 100 mbps (constant across experiments)
...
//
  ...
  p2p.EnablePcapAll ("module03_lab00");
  ...
  //  connfection from N1 to N3

  uint16_t sinkPort2 = 6;
  Address sinkAddress2 (InetSocketAddress (i3i5.GetAddress (0), sinkPort2)); // interface of n3

    // Change the first argument of PacketSinkHelper2 from ns3::UdpSocketFactory to ns3::TcpSocketFactory for TCP
  ...
  sinkApps2.Stop (Seconds (100.));
    ...
    //Change ns3UdpSocket to TCP socket with ns3TcpSocket and give TCP arguments TcpSocketFactory intead of Udp

  Ptr<Socket> ns3TcpSocket = Socket::CreateSocket (c.Get (1), TcpSocketFactory::GetTypeId ()); //source at n1

  // Create  application at n1
  Ptr<MyApp> app2 = CreateObject<MyApp> ();

  //  change Setup argument from ns3UdpSocket to ns3TcpSocket
  app2->Setup (ns3TcpSocket, sinkAddress2, 1040, 100000, DataRate ("250Kbps"));
